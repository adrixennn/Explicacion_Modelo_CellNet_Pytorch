...
* En cada iteración de los folds, he guardado los errores en una matriz cuyas filas correspondían a cada fold y las columnas a las épocas (una matriz de 5x100). Para generar la gráfica lineal con "Matplotlib", fue necesario transponer la matriz. Esto se debe a que la función de graficado interpreta el índice de las filas como el eje X (las épocas) y cada columna como una serie de datos distinta (cada fold).
